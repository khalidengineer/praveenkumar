...
function addToCart(name, price, img, id = null) {
  ...
  saveCart(cart);
  showToast(`Added "${name}" (Rs. ${price}) to Bag! ✨`);
...
function updateBadges() {
  ...
  document.querySelectorAll('.cart-header-total').forEach(el => {
    el.innerText = `Rs. ${total.toLocaleString()}`;
  });
  ...
  if (drawerSubtotal) drawerSubtotal.innerText = `Rs. ${total.toLocaleString()}.00`;
}
  ...
  container.innerHTML = cart.map(item => `
    <div class="flex gap-3.5 pb-4 border-b border-stone-100 items-center">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="text-xs font-bold text-livora-dark mt-0.5">Rs. ${item.price.toLocaleString()}</div>
    ...
    <div id="mobile-filter-panel" class="absolute left-0 right-0 bottom-0 bg-white rounded-t-2xl shadow-2xl p-5 max-h-[80vh] flex flex-col justify-between transform translate-y-full transition-transform duration-300 pointer-events-auto overflow-y-auto">
      ...
      <div class="py-4 space-y-5 text-xs">
        ...
        <div>
          ...
          <div class="grid grid-cols-2 gap-2 text-stone-600">
            <label class="flex items-center gap-2 p-2 bg-stone-50 rounded-lg"><input type="radio" name="mobile_price" /> Under Rs. 200</label>
            <label class="flex items-center gap-2 p-2 bg-stone-50 rounded-lg"><input type="radio" name="mobile_price" /> Rs. 200 - Rs. 500</label>
            <label class="flex items-center gap-2 p-2 bg-stone-50 rounded-lg"><input type="radio" name="mobile_price" checked /> Rs. 500 - Rs. 1,000</label>
            <label class="flex items-center gap-2 p-2 bg-stone-50 rounded-lg"><input type="radio" name="mobile_price" /> Rs. 1,000+</label>
          </div>
        </div>
      </div>
